...
class RummyPool_model extends MY_Model
{
    public function getTableMaster($boot_value='')
    {
        $this->db->select('tbl_rummy_pool_table_master.*,COUNT(tbl_users.id) AS online_members');
        $this->db->from('tbl_rummy_pool_table_master');
        $this->db->join('tbl_rummy_table', 'tbl_rummy_pool_table_master.boot_value=tbl_rummy_table.boot_value AND tbl_rummy_table.isDeleted=0', 'left');
        $this->db->join('tbl_users', 'tbl_users.rummy_pool_table_id=tbl_rummy_table.id AND tbl_users.isDeleted=0', 'left');
        // $this->db->where('', false);
        // $this->db->where('tbl_users.table_id!=', 0);
        if (!empty($boot_value)) {
            $this->db->where('tbl_rummy_pool_table_master.boot_value', $boot_value);
        }
        $this->db->where('tbl_rummy_pool_table_master.isDeleted', 0);
        $this->db->group_by('tbl_rummy_pool_table_master.boot_value');
        $this->db->order_by('tbl_rummy_pool_table_master.boot_value');
        $Query = $this->db->get();
    ...
    {
        $this->db->select('tbl_users.rummy_pool_table_id,COUNT(tbl_users.id) AS members,tbl_rummy_pool_table.private,tbl_rummy_pool_table.boot_value');
        $this->db->from('tbl_users');
        $this->db->join('tbl_rummy_pool_table', 'tbl_users.rummy_pool_table_id=tbl_rummy_pool_table.id');
        $this->db->where('tbl_users.isDeleted', false);
        // $this->db->where('tbl_rummy_pool_table.private', false);
        $this->db->where('tbl_users.rummy_pool_table_id!=', 0);
        $this->db->group_by('tbl_users.rummy_pool_table_id');
        $Query = $this->db->get();
        return $Query->result();
    }
    ...
    {
        $this->db->select('tbl_rummy_pool_table.*,COUNT(tbl_users.id) AS online_members');
        $this->db->from('tbl_rummy_pool_table');
        $this->db->join('tbl_users', 'tbl_users.rummy_pool_table_id=tbl_rummy_pool_table.id AND tbl_users.isDeleted=0', 'LEFT');
        // $this->db->where('tbl_rummy_pool_table.private', false);
        $this->db->where('tbl_rummy_pool_table.winning_amount', 0);
        $this->db->where('tbl_rummy_pool_table.boot_value!=', 0);
        $this->db->group_by('tbl_users.rummy_pool_table_id');
        $Query = $this->db->get();
        // echo $this->db->last_query();
        return $Query->result();
    ...
    {
        $this->db->select('rummy_pool_table_id');
        $this->db->from('tbl_users');
        $this->db->where('isDeleted', false);
        $this->db->where('rummy_pool_table_id', $TableId);
        $Query = $this->db->get();
        return $Query->row();
    }
    ...
    {
        $sql = "SELECT * FROM ( SELECT 1 AS mycolumn UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 ) a WHERE mycolumn NOT in ( SELECT seat_position FROM `tbl_rummy_pool_table_user` WHERE table_id=" . $TableId . " AND isDeleted=0 ) LIMIT 1";
    ...
    {
        $this->db->select('tbl_rummy_pool_table_user.*,tbl_users.name,tbl_users.mobile,tbl_users.profile_pic,tbl_users.wallet,tbl_users.user_type,tbl_users.fcm');
        $this->db->from('tbl_rummy_pool_table_user');
        $this->db->join('tbl_users', 'tbl_rummy_pool_table_user.user_id=tbl_users.id');
        $this->db->where('tbl_rummy_pool_table_user.isDeleted', false);
        $this->db->where('tbl_rummy_pool_table_user.table_id', $TableId);
        $this->db->order_by('tbl_rummy_pool_table_user.seat_position', 'asc');
        $Query = $this->db->get();
        return $Query->result();
    }
    ...
    {
        $this->db->select('tbl_rummy_pool_table_user.*,tbl_users.name,tbl_users.mobile,tbl_users.profile_pic,tbl_users.wallet,tbl_users.user_type');
        $this->db->from('tbl_rummy_pool_table_user');
        $this->db->join('tbl_users', 'tbl_rummy_pool_table_user.user_id=tbl_users.id');
        $this->db->where('tbl_rummy_pool_table_user.isDeleted', false);
        $this->db->where('tbl_rummy_pool_table_user.table_id', $TableId);
        $this->db->order_by('tbl_rummy_pool_table_user.card_position', 'DESC');
        $Query = $this->db->get();
        return $Query->result();
    }
    ...
    public function getGameBot($game_id)
    {
        ...
        $this->db->from('tbl_users');
        $this->db->join('tbl_rummy_pool_card', 'tbl_rummy_pool_card.user_id=tbl_users.id');
        $this->db->where('tbl_users.mobile', "");
        $this->db->where('tbl_rummy_pool_card.packed', false);
    ...
    {
        $this->db->select('tbl_rummy_pool_card.*,tbl_users.name,tbl_users.user_type');
        $this->db->from('tbl_rummy_pool_card');
    ...
    {
        $Query = $this->db->query("SELECT sum(tbl_rummy_pool_log.amount) as amount,tbl_rummy_pool_table_user.seat_position,tbl_rummy_pool_table_user.user_id FROM `tbl_rummy_pool_table_user` JOIN tbl_rummy_pool ON tbl_rummy_pool_table_user.table_id=tbl_rummy_pool.table_id JOIN tbl_rummy_pool_log ON tbl_rummy_pool.id=tbl_rummy_pool_log.game_id AND tbl_rummy_pool_log.user_id=tbl_rummy_pool_table_user.user_id WHERE tbl_rummy_pool_table_user.table_id='".$table_id."' AND tbl_rummy_pool_table_user.seat_position='".$seat_position."' AND tbl_rummy_pool_log.amount>0");
        return $Query->row();
    }
    ...
    public function getMyCards($game_id, $user_id, $card='')
    {
        ...
        // $this->db->update('tbl_rummy_pool_card');  //table name
    ...
    public function ChatList($rummy_pool_table_id, $limit='')
    {
        ...
        $this->db->where('tbl_users.rummy_pool_table_id', $rummy_pool_table_id);
    ...
    {
        $this->db->insert('tbl_rummy_pool', $data);
    ...
    {
        $this->db->insert('tbl_rummy_pool_table', $data);
    ...
    {
        $this->db->select('tbl_users.rummy_pool_table_id,COUNT(tbl_users.id) AS members');
        $this->db->from('tbl_users');
        $this->db->join('tbl_rummy_pool_table', 'tbl_users.rummy_pool_table_id=tbl_rummy_pool_table.id');
        $this->db->where('tbl_users.isDeleted', false);
        // $this->db->where('tbl_table.private', 2);
        $this->db->where('tbl_rummy_pool_table.boot_value', $boot_value);
        $this->db->where('tbl_users.rummy_pool_table_id!=', 0);
        $this->db->group_by('tbl_users.rummy_pool_table_id');
        $Query = $this->db->get();
        return $Query->result();
    ...
    {
        $this->db->insert('tbl_rummy_pool_table_user', $data);
        $TableId =  $this->db->insert_id();

        $this->db->set('rummy_pool_table_id', $data['table_id']); //value that used to update column
    ...
    public function RemoveTableUser($data)
    {
        ...
        $this->db->update('tbl_rummy_pool_table_user');  //table name
    ...
    {
        $this->db->insert('tbl_rummy_pool_card', $data);
        $TableId =  $this->db->insert_id();
    ...
    {
        $this->db->select('tbl_rummy_pool_log.*');
        $this->db->from('tbl_rummy_pool_log');
        $this->db->join('tbl_rummy_pool', 'tbl_rummy_pool.id=tbl_rummy_pool_log.game_id');
        $this->db->where('tbl_rummy_pool.table_id', $table_id);
        $this->db->where('tbl_rummy_pool_log.user_id', $user_id);
        $this->db->where('tbl_rummy_pool_log.amount!=', 0);
        $this->db->where('tbl_rummy_pool_log.points!=', 0);
        $this->db->order_by('tbl_rummy_pool_log.id', 'DESC');
        $this->db->limit(1);
    ...
    public function Rejoin($data)
    {
        ...
        $this->db->order_by('id', 'DESC');
        $this->db->limit(1);
        return $this->db->update('tbl_rummy_pool_log');
    }
    ...
    public function getTotalPoints($table_id, $user_id)
    {
        ...
        $this->db->where('table_id', $table_id);
        $this->db->where('user_id', $user_id);
        return $this->db->get('tbl_rummy_pool_table_user')->row()->total_points;
    }

    public function Update($data, $game_id)
    {
        $this->db->where('id', $game_id);
        $this->db->update('tbl_rummy_pool', $data);
    ...
    {
        $this->db->select('tbl_rummy_pool.*');
        $this->db->from('tbl_rummy_pool');
        $this->db->where('isDeleted', false);
        $this->db->where('tbl_rummy_pool.id', $id);
    ...
    public function Delete($id)
    {
        ...
        $return = $this->db->update('tbl_rummy_pool');  //table name
    ...
    public function DeleteTable($id)
    {
        ...
        $this->db->set('isDeleted', true); //value that used to update column
        $this->db->where('id', $id); //which row want to upgrade
        $return = $this->db->update('tbl_rummy_pool_table');  //table name

        $this->db->set('rummy_pool_table_id', 0); //value that used to update column
        $this->db->where('rummy_pool_table_id', $id); //which row want to upgrade
    ...
    {
        $Query = $this->db->select('SUM(tbl_rummy_pool.amount) as Total_Win,tbl_rummy_pool.winner_id,tbl_users.name,tbl_users.profile_pic')
            ->from('tbl_rummy_pool')
            ->join('tbl_users', 'tbl_users.id=tbl_rummy_pool.winner_id')
            ->where('tbl_rummy_pool.winner_id!=', 0)
            ->group_by('tbl_rummy_pool.winner_id')
            ->order_by('SUM(tbl_rummy_pool.amount)', 'desc')
            ->limit(50)
            ->get();
        // echo $this->db->last_query();
        // exit;
    ...
    public function GetRamdomGameCard($game_id)
    {
        ...
        $this->db->from('tbl_cards_rummy');
        $this->db->where('`cards` NOT IN (SELECT `joker` FROM `tbl_rummy_pool` WHERE `id`='.$game_id.')', null, false);
        $this->db->where('`cards` NOT IN (SELECT `card` FROM `tbl_rummy_pool_card` WHERE `game_id`='.$game_id.' AND isDeleted=0)', null, false);
        $this->db->where('`cards` NOT IN (SELECT `card` FROM `tbl_rummy_pool_card_drop` WHERE `game_id`='.$game_id.' AND isDeleted=0)', null, false);
        $this->db->limit(1);
        $this->db->order_by('RAND()');
        $Query = $this->db->get();
        // echo $this->db->last_query();
    ...
    public function deleteDropCard($game_id)
    {
        ...
        $data = ['isDeleted' => 1, 'updated_date' => date('Y-m-d H:i:s')];
        $this->db->update('tbl_rummy_pool_card_drop', $data, $where);
    }

    public function GetGameDropCard($game_id)
    {
        $this->db->select('card');
        $this->db->from('tbl_rummy_pool_card_drop');
        $this->db->where('game_id', $game_id);
        $this->db->where('game_id', $game_id);
        $this->db->limit(1);
        $this->db->order_by('id', 'DESC');
    ...
    {
        $this->db->select('tbl_rummy_pool_log.game_id,tbl_rummy_pool_log.user_id,tbl_rummy_pool_log.points,tbl_rummy_pool_log.total_points,tbl_users.name');
        $this->db->from('tbl_rummy_pool_log');
    ...
    public function ChangeCard($game_id, $User_id, $Position, $Card)
    {
        ...
        $this->db->where('game_id', $game_id);
        $this->db->where('user_id', $User_id);
        $Update = $this->db->update('tbl_rummy_pool_card', $data);
        if ($Update) {
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('tbl_rummy_pool_table_user', $data);
        $GameId =  $this->db->affected_rows();
        // echo $this->db->last_query();
        return $GameId;
    }
