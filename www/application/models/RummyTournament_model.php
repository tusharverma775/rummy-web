...
class RummyTournament_model extends MY_Model
{
    ...
    {
        $this->db->select('tbl_users.rummy_tournament_table_id,COUNT(tbl_users.id) AS members,tbl_rummy_tournament_table.private,tbl_rummy_tournament_table.boot_value');
        $this->db->from('tbl_users');
        $this->db->join('tbl_rummy_tournament_table', 'tbl_users.rummy_tournament_table_id=tbl_rummy_tournament_table.id');
        $this->db->where('tbl_users.isDeleted', false);
        // $this->db->where('tbl_rummy_tournament_table.private', false);
        $this->db->where('tbl_users.rummy_tournament_table_id!=', 0);
        $this->db->group_by('tbl_users.rummy_tournament_table_id');
        $Query = $this->db->get();
        return $Query->result();
    }
    ...
    {
        $this->db->select('tbl_users.rummy_tournament_table_id,COUNT(tbl_users.id) AS members');
        $this->db->from('tbl_users');
        $this->db->join('tbl_rummy_tournament_table', 'tbl_users.rummy_tournament_table_id=tbl_rummy_tournament_table.id');
        $this->db->where('tbl_users.isDeleted', false);
        $this->db->where('tbl_rummy_tournament_table.private', false);
    ...
    public function getCustomizeActiveTable($tournament_id)
    {
        $this->db->select('tbl_users.rummy_tournament_table_id,COUNT(tbl_users.id) AS members');
        $this->db->from('tbl_users');
        $this->db->join('tbl_rummy_tournament_table', 'tbl_users.rummy_tournament_table_id=tbl_rummy_tournament_table.id');
        ...
        $this->db->where('tbl_rummy_tournament_table.tournament_id', $tournament_id);
        $this->db->where('tbl_users.rummy_tournament_table_id!=', 0);
        $this->db->group_by('tbl_users.rummy_tournament_table_id');
        $Query = $this->db->get();
        return $Query->result();
    ...
    public function isTournament($tournament_id)
    {
        $this->db->from('tbl_rummy_tournament_master');
    ...
    public function alreadyRegistered($user_id, $tournament_id)
    {
        $this->db->from('tbl_rummy_tournament_participants');
        $this->db->where('isDeleted', false);
    ...
    {
        $this->db->select('tbl_rummy_tournament_participants.*,tbl_users.name');
        $this->db->join('tbl_users', 'tbl_rummy_tournament_participants.user_id=tbl_users.id');
        $this->db->where('tbl_rummy_tournament_participants.isDeleted', false);
        $this->db->where('tbl_rummy_tournament_participants.tournament_id', $tournament_id);
        $Query = $this->db->get('tbl_rummy_tournament_participants');
        return $Query->result();
    }

    public function AddTournamentUser($data)
    {
        $this->db->insert('tbl_rummy_tournament_participants', $data);
    ...
    {
        $this->db->select('rummy_tournament_table_id');
        $this->db->from('tbl_users');
        $this->db->where('isDeleted', false);
        $this->db->where('rummy_tournament_table_id', $TableId);
        $Query = $this->db->get();
        return $Query->row();
    }
    ...
    {
        $this->db->from('tbl_rummy_tournament_card');
    ...
    public function getGameBot($game_id)
    {
        ...
        $this->db->from('tbl_users');
        $this->db->join('tbl_rummy_tournament_card', 'tbl_rummy_tournament_card.user_id=tbl_users.id');
        $this->db->where('tbl_users.mobile', "");
        $this->db->where('tbl_rummy_tournament_card.packed', false);
        $this->db->where('tbl_rummy_tournament_card.game_id', $game_id);
        $Query = $this->db->get();
        return $Query->row()->id;
    }
    ...
    {
        $return = false;
        $this->db->from('tbl_rummy_tournament_log');
        $this->db->where('user_id', $user_id);
        $this->db->order_by('id', 'DESC');
        $Query = $this->db->get();
    ...
    {
        $this->db->select('tbl_rummy_tournament_card.*,tbl_users.name,tbl_users.user_type');
        $this->db->from('tbl_rummy_tournament_card');
        $this->db->join('tbl_users', 'tbl_users.id=tbl_rummy_tournament_card.user_id');
        $this->db->where('tbl_rummy_tournament_card.game_id', $game_id);
        $this->db->group_by('tbl_rummy_tournament_card.user_id');
        $Query = $this->db->get();
        // echo $this->db->last_query();
        return $Query->result();
    }
    ...
    {
        $this->db->select('tbl_rummy_tournament_card.user_id,tbl_rummy_tournament_card.packed,tbl_users.name');
        $this->db->from('tbl_rummy_tournament_card');
        $this->db->join('tbl_users', 'tbl_users.id=tbl_rummy_tournament_card.user_id');
        $this->db->where('tbl_rummy_tournament_card.game_id', $game_id);
        $this->db->group_by('tbl_rummy_tournament_card.user_id');
    ...
    public function getMyCards($game_id, $user_id, $card='')
    {
        ...
        // $this->db->update('tbl_rummy_tournament_card');  //table name
    ...
    public function GetGameCard($game_id)
    {
        ...
        $this->db->where('`cards` NOT IN (SELECT `joker` FROM `tbl_rummy_tournament` WHERE `id`='.$game_id.')', null, false);
        $this->db->where('`cards` NOT IN (SELECT `card` FROM `tbl_rummy_tournament_card` WHERE `game_id`='.$game_id.' AND isDeleted=0)', null, false);
        $this->db->where('`cards` NOT IN (SELECT `card` FROM `tbl_rummy_tournament_card_drop` WHERE `game_id`='.$game_id.' AND isDeleted=0)', null, false);
    ...
    public function SwapCards($user_id, $game_id, $my_card, $new_card)
    {
        ...
        $this->db->update('tbl_rummy_tournament_card', $data, $where);
        ...
        $this->db->insert('tbl_rummy_tournament_card', $table_user_data);
        $TableId =  $this->db->insert_id();
        // echo $this->db->last_query();
        // exit;
        return $TableId;
    ...
    {
        $this->db->insert('tbl_rummy_tournament_table_user', $data);
        $TableId =  $this->db->insert_id();

        $this->db->set('rummy_tournament_table_id', $data['table_id']); //value that used to update column
    ...
    public function RemoveTableUser($data)
    {
        ...
        $this->db->update('tbl_rummy_tournament_table_user');  //table name
    ...
    public function PackGame($user_id, $game_id, $timeout = 0, $json = '', $amount = 0, $percent = 0)
    {
        ...
        $this->db->insert('tbl_rummy_tournament_log', $data);
        return true;
    }
    ...
    {
        $this->db->insert('tbl_rummy_tournament_card', $data);
        $TableId =  $this->db->insert_id();
        // echo $this->db->last_query();
        return $TableId;
    }
    ...
    {
        $this->db->select('tbl_rummy_tournament.*');
        $this->db->from('tbl_rummy_tournament');
        $this->db->where('isDeleted', false);
        $this->db->where('tbl_rummy_tournament.id', $id);
    ...
    public function Delete($id)
    {
        ...
        $return = $this->db->update('tbl_rummy_tournament');  //table name
    ...
    public function DeleteTable($id)
    {
        ...
        $this->db->set('isDeleted', true); //value that used to update column
        $this->db->where('id', $id); //which row want to upgrade
        $return = $this->db->update('tbl_rummy_tournament_table');  //table name

        $this->db->set('rummy_tournament_table_id', 0); //value that used to update column
        $this->db->where('rummy_tournament_table_id', $id); //which row want to upgrade
    ...
    {
        $Query = $this->db->select('SUM(tbl_rummy_tournament.amount) as Total_Win,tbl_rummy_tournament.winner_id,tbl_users.name,tbl_users.profile_pic')
            ->from('tbl_rummy_tournament')
            ->join('tbl_users', 'tbl_users.id=tbl_rummy_tournament.winner_id')
            ->where('tbl_rummy_tournament.winner_id!=', 0)
            ->group_by('tbl_rummy_tournament.winner_id')
            ->order_by('SUM(tbl_rummy_tournament.amount)', 'desc')
            ->limit(50)
            ->get();
        // echo $this->db->last_query();
        // exit;
    ...
    public function GetAndDeleteGameDropCard($game_id)
    {
        ...
        if ($result) {
            ...
            $data = ['isDeleted' => 1, 'updated_date' => date('Y-m-d H:i:s')];
            $this->db->update('tbl_rummy_tournament_card_drop', $data, $where);
        }
        // echo $this->db->last_query();
        // exit;
        return $result;
    ...
    public function ChangeCard($game_id, $User_id, $Position, $Card)
    {
        ...
        $this->db->where('game_id', $game_id);
        $this->db->where('user_id', $User_id);
        $Update = $this->db->update('tbl_rummy_tournament_card', $data);
        if ($Update) {
    ...
    {
        $this->db->select('tbl_rummy_tournament.*,tbl_users.name');
        $this->db->from('tbl_rummy_tournament');
        $this->db->join('tbl_users', 'tbl_users.id=tbl_rummy_tournament.winner_id', 'left');
        $this->db->order_by('tbl_rummy_tournament.id', 'DESC');
        $this->db->limit(10);
        $Query = $this->db->get();
        // echo $this->db->last_query();
        // die();
